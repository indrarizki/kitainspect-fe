<template>
    <div>
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px">
            <div>
                <h1 style="font-size:16px;font-weight:500">Manajemen Department</h1>
                <p style="font-size:12px;color:var(--color-text-tertiary);margin-top:2px">Kelola data department</p>
            </div>
            <div>
                <button v-if="can('department.create')" class="btn btn-primary" @click="openModal()">+ Tambah Department</button>
            </div>
        </div>

        <div class="table-wrap">
            <div v-if="loading" class="loading-ph">Memuat...</div>
            <div v-else-if="!items.length" class="empty-ph">Tidak ada department ditemukan.</div>
            <table v-else>
                <thead>
                <tr>
                    <th>Code</th>
                    ...
                    <tr v-for="c in items" :key="c.id">
                        ...
                        <td>
                            <div style="display:flex;gap:5px">
                                <button class="btn btn-secondary btn-sm" @click="$router.push(`/departments/${c.id}`)">Edit</button>
                            </div>
                        </td>
                    </tr>
                </tbody>
  ...
  <div v-if="showAddModal" class="modal-overlay">
            <div class="modal">
                <div class="modal-title">Tambah Department</div>
                <div style="margin-top:12px">
                    ...
                    <label style="margin-top:10px;display:block">Department Name</label>
                    <input v-model="form.name" style="width:100%;padding:8px;margin-top:6px;border:1px solid var(--color-border-tertiary);border-radius:6px" />
...
function closeAddModal() { showAddModal.value = false; form.value = { code:'', name:'', status:'active' } }

async function submitAdd() {
    if (!form.value.code || !form.value.name) return alert('Isi semua field')
    savingAdd.value = true
    try {
        await departmentService.create({ code: form.value.code, name: form.value.name, status: form.value.status })
